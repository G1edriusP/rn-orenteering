...
const FilterButton: React.FC<FilterButtonProps> = ({
  ...
  return (
    ...
    >
      ...
      <Text
        style={[
          isActive ? styles.activeTitle : styles.disabledTitle,
          { marginTop: Icon && 12 },
        ]}
      >
        {title}
...
const TracksScreen = ({ route: { params }, navigation }: TracksScreenProps) => {
  ...
  const onFilterValueChange = useCallbackOne(
    (type: string, value: string | []) => {
      if (selectedFilters.isDefault)
        dispatch({ type: 'isDefault', value: false });
      ...
      if (selectedFilters.isDefault) {
        ...
        const filteredTracks = tracks.filter((track) => {
          return (
            ...
            (filters.duration
              ? track.duration / 3600 >= filters.duration[0] &&
                track.duration / 3600 <= filters.duration[1]
              : true) &&
            (filters.rating
              ? track.rating >=
                filters.rating[selectedFilters.rating.length - 1] + 1
              : true)
          );
        });
  ...
  return (
    ...
    >
      ...
      <TracksList
        tracks={routes}
        title={t('tracks:emptyTracks')}
        onPress={openTrackInfo}
      />
        ...
        <SafeAreaView edges={['bottom']} style={styles.wrapShadow}>
          <View style={styles.footerWrap}>
            <Button
              ...
              onPress={() =>
                onFilterValueChange('RESET', findTracksMinMaxDuration(routes))
              }
              style={styles.footerFlex}
              textStyle={{ color: colors.BLACK }}
            />
            <Text
              style={{ fontFamily: fontRegular, fontSize: fontSizes.SMALL }}
            >
